...
public interface ClaimedChunk {
    ...
    ///
    ///     **IMPORTANT**: this does not guarantee that the chunk is actually currently being force-loaded
    ///     by the server. That depends on the offline chunkloading settings, and whether the chunk owner is actually online.
    ///     See [#isActuallyForceLoaded()] for a method to check that.
    ///
    ...
    /// Convenience method to check if the chunk has force-loading enabled; equivalent to: `if (getForceLoadedTime() > 0)`
    ///
    ...
    /// a time.
    ///
    /// @param now the current time, as returned by `System.getCurrentTimeMillis()`
