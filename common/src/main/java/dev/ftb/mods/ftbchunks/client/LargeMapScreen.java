...
public class LargeMapScreen extends BaseScreen {
	...
	public void addWidgets() {
		...
		add(settingsButton = new SimpleButton(this, new TranslatableComponent("ftbchunks.gui.settings"), Icons.SETTINGS, (b, m) -> FTBChunksClientConfig.openSettings(new ScreenWrapper(this))));

		if (Minecraft.getInstance().player.hasPermissions(2)) {
			add(serverSettingsButton = new SimpleButton(this, new TranslatableComponent("ftbchunks.gui.settings.server"), Icons.SETTINGS.withTint(Color4I.rgb(0xA040FF)), (b, m) -> FTBChunksClientConfig.openServerSettings(new ScreenWrapper(this))));
		}
	}
	...
	public void drawForeground(PoseStack matrixStack, Theme theme, int x, int y, int w, int h) {
		...
		if (zoom == minZoom && zoom > 1) {
			Component zoomWarn = new TranslatableComponent("ftbchunks.zoom_warning");
			matrixStack.pushPose();
