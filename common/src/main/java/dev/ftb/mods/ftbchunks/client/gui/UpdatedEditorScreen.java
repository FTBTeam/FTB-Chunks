...
public class UpdatedEditorScreen extends AbstractButtonListScreen {
    ...
    public void addButtons(Panel panel) {
        for (Map.Entry<ResourceKey<Level>, List<WaypointImpl>> resourceKeyListEntry : waypoints.entrySet()) {
            boolean startCollapsed = collapsed.get(resourceKeyListEntry.getKey());
            GroupButton groupButton = new GroupButton(panel, resourceKeyListEntry.getKey(), startCollapsed, resourceKeyListEntry.getValue());
    ...
    private class GroupButton extends Button {
        ...
        public List<RowPanel> collectPanels() {
            List<RowPanel> panels = new ArrayList<>();
            if(!isCollapsed()) {
                panels.addAll(rowPanels);
            }
            return panels;
        }
        ...
        public void setCollapsed(boolean collapsed) {
            UpdatedEditorScreen.this.collapsed.put(dim, collapsed);
    ...
    private class RowPanel extends Panel {
        ...
        public void addWidgets() {
            ...
            add(nameField = new TextField(this).setTrim().setText(wp.getName()).setColor(Color4I.rgb(wp.getColor())).addFlags(Theme.SHADOW));
        ...
        @Override
        public void alignWidgets() {
            //Todo names can be to long same with distance (also right aline this one)
            System.out.println("This: " + this.getWidth());
            System.out.println("Parent: " + parent.getWidth());
            boolean shouldDraw = parent.getWidth() == 202;
            int farRight = parent.getWidth() + 2 - 12;
            System.out.println(parent.getWidth());
//            farRight = parent.getWidth() - scrollBar.getScrollBarSize();
//            int farRight = (scrollBar.shouldDraw() ? getGui().width - getScrollbarWidth() - 2 : getGui().width) - mainPanelInset().getFirst() * 2;
            int yOff = (this.height - getTheme().getFontHeight()) / 2 + 1;
            hideButton.setPos(farRight - 8 - 16, 1);
            nameField.setMaxWidth(64);
            nameField.setPos(5, yOff);
            distField.setPos(farRight - 8 - 56, yOff);
//            TooltipList list = new TooltipList();
//            list.add(Component.literal("X: " + wp.getPos().getX() + " Y: " + wp.getPos().getY() + " Z: " + wp.getPos().getZ()));
//            distField.addMouseOverText(list);
        ...
        private void deleteWaypoint(boolean gui) {
            if(gui) {
                getGui().openYesNo(Component.translatable("ftbchunks.gui.delete_waypoint", Component.literal(wp.getName())
                    ...
                    UpdatedEditorScreen.this.waypoints.get(wp.getDimension()).remove(wp);
                    getGui().refreshWidgets();
                });
            }else {
                wp.removeFromManager();
    ...
    private static Map<ResourceKey<Level>, List<WaypointImpl>> collectWaypoints() {
        ...
        manager.getDimensions().values().stream()
                ...
                .forEach(dim -> {
                            res.put(dim.dimension, dim.getWaypointManager().stream()
