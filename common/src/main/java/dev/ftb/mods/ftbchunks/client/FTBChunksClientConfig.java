...
public interface FTBChunksClientConfig {
	...
	DoubleValue NOISE = CONFIG.addDouble("noise", 0.05D, 0D, 0.5D).fader().comment("Noise added to map to make it look less plastic");
	DoubleValue SHADOWS = CONFIG.addDouble("shadows", 0.1D, 0D, 0.3D).fader().comment("Shadow intensity");
	BooleanValue CHUNK_GRID = CONFIG.addBoolean("chunk_grid", false).comment("Chunk grid overlay in large map");
	BooleanValue REDUCED_COLOR_PALETTE = CONFIG.addBoolean("reduced_color_palette", false).comment("Reduces color palette to 256 colors");
	DoubleValue SATURATION = CONFIG.addDouble("saturation", 1D, 0D, 1D).fader().comment("Color intensity");
	BooleanValue CLAIMED_CHUNKS_ON_MAP = CONFIG.addBoolean("claimed_chunks_on_map", true).comment("Show claimed chunks on the map");
	BooleanValue OWN_CLAIMED_CHUNKS_ON_MAP = CONFIG.addBoolean("own_claimed_chunks_on_map", true).comment("Show your own claimed chunks on the map");
	BooleanValue ONLY_SURFACE_ENTITIES = CONFIG.addBoolean("only_surface_entities", true).comment("Only show entities that are on the surface");
	EnumValue<MapMode> MAP_MODE = CONFIG.addEnum("map_mode", MapMode.NAME_MAP).comment("Different ways to render map");
	IntValue WATER_HEIGHT_FACTOR = CONFIG.addInt("water_height_factor", 8, 0, 128).comment("How many blocks should height checks skip in water. 0 means flat water, ignoring terrain");
	EnumValue<BiomeBlendMode> BIOME_BLEND = CONFIG.addEnum("biome_blend", BiomeBlendMode.NAME_MAP).comment("Biome blend");
	IntValue WATER_VISIBILITY = CONFIG.addInt("water_visibility", 220, 0, 255).excluded().comment("Advanced option. Water visibility");
	IntValue GRASS_DARKNESS = CONFIG.addInt("grass_darkness", 50, 0, 255).excluded().comment("Advanced option. Grass darkness");
	IntValue FOLIAGE_DARKNESS = CONFIG.addInt("foliage_darkness", 50, 0, 255).excluded().comment("Advanced option. Foliage darkness");

	SNBTConfig WAYPOINTS = CONFIG.addGroup("waypoints");
	...
	SNBTConfig MINIMAP = CONFIG.addGroup("minimap");
	...
	SNBTConfig MEMORY = CONFIG.addGroup("memory");
