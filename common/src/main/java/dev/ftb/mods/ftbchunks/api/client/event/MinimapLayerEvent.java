 ...
 */
public class MinimapLayerEvent {
    public static final Event<Consumer<MinimapLayerEvent>> ADD_LAYERS = EventFactory.createConsumerLoop();

    private final Consumer<PositionedLayer> callback;

    @ApiStatus.Internal
    public MinimapLayerEvent(Consumer<PositionedLayer> callback) {
     ...
     * {@link dev.ftb.mods.ftbchunks.api.client.minimap.DefaultRenderLayers} for a collection of the layer IDs which
     ...
     */
    public record Order(boolean after, @Nullable Identifier otherLayer) {
    ...
    public record PositionedLayer(Identifier id, MinimapLayerRenderer renderer, Order order) {
    }
}
