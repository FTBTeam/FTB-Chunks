...
public class PermissionsHelper {
    ...
    public static int getMaxClaimedChunks(ServerPlayer player, int def) {
        return PermissionHelper.getProvider().getIntegerPermission(player, MAX_CLAIMED_PERM, def);
    }

    public static int getMaxForceLoadedChunks(ServerPlayer player, int def) {
        return PermissionHelper.getProvider().getIntegerPermission(player, MAX_FORCE_LOADED_PERM, def);
    }

    public static boolean getChunkLoadOffline(ServerPlayer player, boolean def) {
        return PermissionHelper.getProvider().getBooleanPermission(player, CHUNK_LOAD_OFFLINE_PERM, def);
    }

    public static boolean getNoWilderness(ServerPlayer player, boolean def) {
        return PermissionHelper.getProvider().getBooleanPermission(player, NO_WILDERNESS_PERM, def);
    }
}
